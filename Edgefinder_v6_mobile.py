...
# One card per matchup • Vertical scroll • Color-coded Play/Neutral/Pass
# Search bar • Local timestamps • DK odds inline • Reason text for every call
...
.note { font-size:12px; color:#b6beca; margin-top:4px; }
...
# Show rules so flips make sense when you move the slider
st.caption(
    f"Rules: **Play** if Edge ≥ {play_thr:.1f}% • **Neutral** if |Edge| ≤ {neutral_band:.1f}% • otherwise **Pass**."
)
...
def build_row(label, game, soccer, play_thr, neutral_band):
    ...
    # Reason text (clear explanation)
    model_pct = model.get(pick_key, 0) * 100
    imp_pct   = implied.get(pick_key, 0) * 100
    if rec == "PLAY":
        reason = (
            f"Value: model {model_pct:.1f}% > market {imp_pct:.1f}% "
            f"(DK {pick_odds:.2f}) by {pick_edge:.1f}%."
        )
    elif rec == "NEUTRAL":
        reason = (
            f"Close: model {model_pct:.1f}% ≈ market {imp_pct:.1f}% "
            f"(±{neutral_band:.1f}%)."
        )
    else:  # PASS
        reason = (
            f"Too short: model {model_pct:.1f}% < market {imp_pct:.1f}% "
            f"at DK {pick_odds:.2f} (−{abs(pick_edge):.1f}%)."
        )
...
def render_card(r: dict):
    ...
    <div class="card">
      ...
      <div class="note"><b>Reason:</b> {r['Reason']}</div>
